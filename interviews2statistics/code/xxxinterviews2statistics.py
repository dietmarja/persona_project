...
def load_maverick_config(config_path):
    with open(config_path, 'r') as file:
        maverick_config = yaml.safe_load(file)
    return maverick_config['maverick']['interjection_probability']
...
def update_results_csv(config, summary_stats, tech_ranking, idea_buildup_ranking, all_results, maverick_probability, maverick_list):
    ...
    # Calculate the average scores for the no_persona condition
    no_persona_results = [result for result in all_results if result['filename'] == 'no_persona_interview.txt']
    no_persona_scores = {
        'tech_score_mean': np.mean([r['tech_score'] for r in no_persona_results]),
        'ethics_score_mean': np.mean([r['ethics_score'] for r in no_persona_results]),
        'education_score_mean': np.mean([r['education_score'] for r in no_persona_results]),
        'idea_buildup_score_mean': np.mean([r['idea_buildup_score'] for r in no_persona_results])
    }
...
def main():
    ...
    for filename in interviews:
        ...
        results = analyze_interview(filename, iterations, config)
        for i, result in enumerate(results):
            result['idea_buildup_score'] = idea_buildup_scores[i]
        all_results.extend(results)

        if not iterations:
            print(f"Warning: No iterations found in {filename}")
        else:
